-- Rook hype outbox + event-ID delivery cursor (review round 2: Codex P1-4 + P2-2, Lucy L5).
--
-- The original rook_hype_events(p_since) had two structural problems:
--   * SCALING: every ~60s poll window-scanned ALL plays in history to compute
--     prior bests (personal_best / bounty) and pair records (rivalry) — right at
--     today's size, wrong at 10x.
--   * DELIVERY: the bot's timestamp cursor advanced past late-committing rows,
--     past the 200-row overflow, and past failed sends — silently lost events.
--
-- This migration replaces the polling contract with an append-only outbox:
--
--   rook_hype_detect(p_lookback)  finds events among plays that OCCURRED in the
--     lookback window and INSERTs them, idempotently (unique event_key). A play
--     that commits late is still inside the lookback on a later run, so late
--     commits are picked up instead of skipped. Expensive prior-max lookups run
--     per NEW candidate play (usually zero per tick), not per poll.
--   rook_hype_fetch(p_limit)      runs detect, expires stale undelivered rows
--     (>24h old — nobody wants day-old hype after an outage), and returns the
--     undelivered backlog in id order.
--   rook_hype_ack(p_ids)          marks rows delivered. The bot acks an event
--     only after Discord accepted the send (or when it deliberately skips it:
--     toggle off, per-player cap, unknown type). A failed send stays unacked and
--     is retried next tick. Delivery is at-least-once by design: a crash between
--     send and ack can repeat ONE event; the old design silently dropped
--     arbitrarily many. For a hype reel, a rare repeat beats a silent hole.
--
-- rook_hype_events(p_since) is KEPT as the read-only replay path (test harness
-- + diag scripts); the live loop no longer calls it.

-- ── Outbox table ─────────────────────────────────────────────────────────────
create table if not exists public.rook_hype_outbox (
  id           bigserial primary key,
  event_key    text not null unique,   -- deterministic identity: type:game:who:when
  type         text not null,
  game         text not null,
  user_id      uuid,
  username     text,
...
create index if not exists rook_hype_outbox_undelivered_idx
  on public.rook_hype_outbox(id) where delivered_at is null and not expired;

-- RLS on, no policies: service_role (via the rook-hype edge fn) only.
alter table public.rook_hype_outbox enable row level security;

-- ── Source-table indexes for the per-candidate lookups ───────────────────────
-- Candidate discovery is a time-range scan; prior-best lookups are per-user
-- range scans. All additive, `if not exists`.
create index if not exists games_finished_at_idx on public.games(finished_at) where finished_at is not null;
...
create index if not exists rg_solo_games_user_played_idx on public.rg_solo_games(user_id, played_at);
create index if not exists rg_solo_games_played_at_idx on public.rg_solo_games(played_at);
create index if not exists rg_games_finished_at_idx on public.rg_games(finished_at) where finished_at is not null;
create index if not exists sn_daily_feeds_user_played_idx on public.sn_daily_feeds(user_id, played_at);
create index if not exists sn_daily_feeds_completed_at_idx on public.sn_daily_feeds(completed_at) where completed_at is not null;
create index if not exists sn_matches_completed_at_idx on public.sn_matches(completed_at) where completed_at is not null;
create index if not exists yahdle_solo_results_user_completed_idx on public.yahdle_solo_results(user_id, completed_at);
create index if not exists yahdle_solo_results_completed_at_idx on public.yahdle_solo_results(completed_at) where completed_at is not null;
create index if not exists yahdle_games_finished_at_idx on public.yahdle_games(finished_at) where finished_at is not null;
create index if not exists oublex_solo_results_completed_at_idx on public.oublex_solo_results(completed_at) where completed_at is not null;

-- ── Detection: insert new events for plays inside the lookback window ────────
...
-- NOTE on the rivalry cand scan: the meet CTE for candidate discovery is bounded
-- by the window predicates pushed into cand; the lateral pair-record aggregate
-- reads only that pair's rows. With zero candidates (the common tick) no pair
-- aggregate runs at all.

-- ── Fetch: detect, expire stale, return the undelivered backlog ──────────────
create or replace function public.rook_hype_fetch(p_limit int default 50)
returns jsonb
language plpgsql
security definer
set search_path = public
as $$
begin
  perform public.rook_hype_detect();

  -- Day-old undelivered hype is stale, not a backlog to flush (a long outage
  -- should not dump yesterday's reel into the channel). Mark it expired — still
  -- in the ledger, never fetched again.
  update rook_hype_outbox
     set expired = true
   where delivered_at is null and not expired
     and occurred_at <= now() - interval '24 hours';

  return coalesce(
    (select jsonb_agg(row_to_json(e)) from (
       select id, type, game, user_id, username, discord_id, detail, occurred_at
       from rook_hype_outbox
       where delivered_at is null and not expired
       order by id
...
-- ── Ack: confirmed-delivery cursor, per event id ──────────────────────────────
create or replace function public.rook_hype_ack(p_ids bigint[])
returns int
language plpgsql
security definer
set search_path = public
...
-- ── Grants: service_role only (bot reaches these via the rook-hype edge fn) ──
revoke all on function public.rook_hype_detect(interval) from public, anon, authenticated;
grant execute on function public.rook_hype_detect(interval) to service_role;
revoke all on function public.rook_hype_fetch(int) from public, anon, authenticated;
grant execute on function public.rook_hype_fetch(int) to service_role;
revoke all on function public.rook_hype_ack(bigint[]) from public, anon, authenticated;
grant execute on function public.rook_hype_ack(bigint[]) to service_role;

-- ── Seed: backfill recent history as already-delivered ───────────────────────
-- The old timestamp cursor lived in bot memory and is gone at cutover. Detect
-- over a generous window, then mark everything delivered, so the first live
-- fetch starts from silence instead of replaying the last two days.
select public.rook_hype_detect(interval '48 hours');
update public.rook_hype_outbox set delivered_at = now() where delivered_at is null;
