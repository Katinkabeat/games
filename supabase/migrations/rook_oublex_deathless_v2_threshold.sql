-- Oublex v2 branching/dice rework (c329): retune the Deathless hype trigger.
--
-- v1 scoring counted overkill and topped out with Deathless at 170+. v2 scores
-- only damage that LANDS, and CLEAR_RANKS in oublexEngine.js was re-derived
-- against the new distribution (scripts/balance-sim.mjs, safe doors, optimal
-- play: min 136 / median 156 / p75 182 / p90 213 / max 259) — Deathless now
-- starts at 210. This migration keeps that literal in sync; it is NOT read
-- from the engine (no shared source of truth across the JS/SQL boundary), so
-- any future CLEAR_RANKS retune must update this threshold too.
--
-- Same function otherwise: full recreate of rook_hype_detect from
-- rook_mouthful_length_only.sql with only block 6's threshold changed. Grants
-- are unchanged by CREATE OR REPLACE (same signature).
...
begin
  ...
  -- 6. OUBLEX DEATHLESS — daily dungeon cleared at the top rank (210+, v2 curve).
  insert into rook_hype_outbox (event_key, type, game, user_id, username, discord_id, detail, occurred_at)
