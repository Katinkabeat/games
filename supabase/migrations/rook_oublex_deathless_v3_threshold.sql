-- Oublex score v3 (2026-08-14): retune the Deathless hype trigger 210 -> 285.
--
-- v2's landed-damage-only score turned out to be monster-HP-determined: every
-- winning run on the same door path scored the same total (144 on risky/risky),
-- so Deathless at 210 could effectively never fire. v3 scores landed damage
-- plus half overkill, plus win-only bonuses (floor(heroHP/2) capped 70, and
-- 3/cast under a 24-cast par). CLEAR_RANKS in oublexEngine.js was re-derived
-- from a WITHIN-day distribution (scripts/balance-sim.mjs, 78 seeded days:
-- min 166 / p75 258 / p90 279 / p95 291 / max 354) -- Deathless now starts at
-- 285 (~p90-95 of strong play). This literal is NOT read from the engine (no
-- shared source of truth across the JS/SQL boundary), so any future
-- CLEAR_RANKS retune must update this threshold too.
--
-- Same function otherwise: full recreate of rook_hype_detect from
-- rook_oublex_deathless_v2_threshold.sql with only block 6's threshold
-- changed. Grants are unchanged by CREATE OR REPLACE (same signature).
