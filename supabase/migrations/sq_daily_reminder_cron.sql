...
-- ── 2. pg_cron schedule ──────────────────────────────────────
-- Runs at HH:00 and HH:30 every hour. The edge function URL +
-- service-role JWT are hardcoded (matches the yahdle push triggers
-- pattern — the URL alone isn't a secret since the function still
-- gates by RPC permission, but the JWT prevents random callers).
...
select cron.schedule(
  ...
  $cron$
    select net.http_post(
      url := 'https://yyhewndblruwxsrqzart.supabase.co/functions/v1/sq-daily-reminder',
