-- sq_pgnet_timeout_15s.sql — 2026-07-14 (c278)
--
-- Raise the pg_net HTTP timeout on every net.http_post call from the 5000ms
-- DEFAULT to 15000ms.
--
-- WHY: the default 5s timeout was cutting off in-flight calls to the push edge
-- functions. Seen repeatedly on 2026-07-14 (net._http_response rows with
-- timed_out = true, "Timeout of 5000 ms reached") — including a single webpush
-- request/response that alone took 4796ms while Mozilla's push service was
-- failing slowly. When it trips, pg_net drops the connection and discards the
-- response, so the trigger goes blind to whether the push landed. These
-- timeouts PRE-DATE the c276 retry (they were firing at 16:30 / 16:39 / 17:01
-- UTC, hours before it deployed); the retry makes them likelier, not causal.
--
-- This costs gameplay NOTHING: net.http_post is ASYNCHRONOUS. It queues the
-- request and returns immediately; a background worker performs it. The trigger
-- (and so the player's move) never blocks on it. The timeout only governs how
-- long that worker waits before giving up, so a longer value buys reliability
-- at no latency cost to anyone.
--
-- Generated by rewriting each function's live pg_get_functiondef() and adding
-- `timeout_milliseconds :=` to each net.http_post call site.
-- 27 functions, 27 call sites.
-- Pre-change snapshot (rollback): supabase/snapshots/http_post_fns_2026-07-14.sql
