-- ============================================================
-- {{name}} — c92 extended leaderboards (Day/Week/Month/All-time)
--
-- Two RPCs, per-user, SUM-across-window:
--   {{slug}}_solo_leaderboard(p_timeframe, p_date)  — top 10
--   {{slug}}_solo_my_rank(p_timeframe, p_date)      — caller's rank+score
--
...
REVOKE ALL ON FUNCTION public.{{slug}}_solo_leaderboard(text, date) FROM public;
GRANT EXECUTE ON FUNCTION public.{{slug}}_solo_leaderboard(text, date) TO authenticated;
...
CREATE OR REPLACE FUNCTION public.{{slug}}_solo_my_rank(
  p_timeframe text,
  p_date      date DEFAULT current_date
)
...
REVOKE ALL ON FUNCTION public.{{slug}}_solo_my_rank(text, date) FROM public;
GRANT EXECUTE ON FUNCTION public.{{slug}}_solo_my_rank(text, date) TO authenticated;
