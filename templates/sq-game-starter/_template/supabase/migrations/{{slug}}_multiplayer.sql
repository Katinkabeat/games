-- ============================================================
-- {{name}} — Multiplayer (N-player: 2–4) schema + RPCs
--
-- Run in Supabase → SQL Editor → New Query. Safe to re-run
-- (idempotent: `create table if not exists`, `create or replace`,
-- drop-policy-first). Run this BEFORE {{slug}}_admin_close_game.sql
-- (which assumes {{slug}}_games / {{slug}}_players already exist),
-- and alongside {{slug}}_nudge.sql.
--
...
--   {{slug}}_submit_turn(p_game_id uuid, p_score int)
...
-- {{slug}}_admin_close_game.sql.
-- ============================================================

-- ── 1. {{slug}}_games ─────────────────────────────────────────
create table if not exists public.{{slug}}_games (
...
do $$ begin
  alter table public.{{slug}}_games
    add constraint {{slug}}_games_current_player_idx_chk
    check (current_player_idx >= 0 and current_player_idx < max_players);
exception when duplicate_object then null; end $$;

create index if not exists {{slug}}_games_status_idx        on public.{{slug}}_games(status);
create index if not exists {{slug}}_games_created_by_idx    on public.{{slug}}_games(created_by);
create index if not exists {{slug}}_games_invited_user_idx  on public.{{slug}}_games(invited_user_id);
...
create table if not exists public.{{slug}}_players (
  id              uuid    primary key default gen_random_uuid(),
  game_id         uuid    not null references public.{{slug}}_games(id) on delete cascade,
...
create table if not exists public.{{slug}}_matchups (
...
-- check in {{slug}}_advance_turn if your game ends a different way.
create or replace function public.{{slug}}_total_turns()
returns int language sql immutable as $$ select 1 $$;

-- ── 5. RLS ────────────────────────────────────────────────────
alter table public.{{slug}}_games    enable row level security;
alter table public.{{slug}}_players  enable row level security;
alter table public.{{slug}}_matchups enable row level security;

-- SECURITY DEFINER membership check — reads {{slug}}_players directly so
-- the games<->players read policies don't recurse into each other.
create or replace function public.{{slug}}_is_participant(p_game_id uuid, p_uid uuid)
returns boolean language sql security definer stable set search_path = public as $$
  select exists (
    select 1 from public.{{slug}}_players
     where game_id = p_game_id and user_id = p_uid
  );
$$;
revoke all on function public.{{slug}}_is_participant(uuid, uuid) from public;
grant execute on function public.{{slug}}_is_participant(uuid, uuid) to authenticated;

drop policy if exists "{{slug}}_games read participant"   on public.{{slug}}_games;
drop policy if exists "{{slug}}_games read open"          on public.{{slug}}_games;
drop policy if exists "{{slug}}_games insert as creator"  on public.{{slug}}_games;
drop policy if exists "{{slug}}_players read participant"  on public.{{slug}}_players;
drop policy if exists "{{slug}}_matchups read own"        on public.{{slug}}_matchups;
...
create policy "{{slug}}_games read participant" on public.{{slug}}_games
  for select using (
    auth.uid() = created_by
    or auth.uid() = invited_user_id
    or auth.uid() = any(coalesce(invited_user_ids, '{}'))
    or public.{{slug}}_is_participant(id, auth.uid())
  );

-- Any waiting game is publicly readable so the lobby can list open +
-- partially-filled games to potential joiners.
create policy "{{slug}}_games read open" on public.{{slug}}_games
  for select using (status = 'waiting');

create policy "{{slug}}_games insert as creator" on public.{{slug}}_games
  for insert with check (auth.uid() = created_by);

-- Players: any seated participant can read every player row in the game.
create policy "{{slug}}_players read participant" on public.{{slug}}_players
  for select using ( public.{{slug}}_is_participant(game_id, auth.uid()) );

-- Matchups: each player reads their own rows.
create policy "{{slug}}_matchups read own" on public.{{slug}}_matchups
...
drop trigger if exists {{slug}}_players_touch_activity on public.{{slug}}_players;
create trigger {{slug}}_players_touch_activity
...
drop trigger if exists {{slug}}_set_game_expiry on public.{{slug}}_games;
create trigger {{slug}}_set_game_expiry
  before insert on public.{{slug}}_games
...
$$;
grant execute on function public.{{slug}}_expire_stale_invites() to authenticated;
...
create or replace function public.{{slug}}_create_game(
...
begin
  ...
  if coalesce(array_length(v_invited, 1), 0) = 0 then
    select count(*) into v_open_count from public.{{slug}}_games
  ...
  insert into public.{{slug}}_games (created_by, invited_user_id, invited_user_ids, max_players, status)
  values (v_uid, v_first, nullif(v_invited, '{}'), p_max_players, 'waiting')
  returning id into v_game_id;

  insert into public.{{slug}}_players (game_id, user_id, player_index)
  values (v_game_id, v_uid, 0);

  return v_game_id;
end;
$$;
grant execute on function public.{{slug}}_create_game(uuid[], int) to authenticated;
...
create or replace function public.{{slug}}_join_game(
  p_game_id uuid
...
begin
  ...
  select * into v_game from public.{{slug}}_games where id = p_game_id for update;
  if not found then raise exception 'Game not found'; end if;
  if v_game.status <> 'waiting' then raise exception 'Game already started or finished'; end if;

  perform 1 from public.{{slug}}_players where game_id = p_game_id and user_id = v_uid;
  if found then raise exception 'Already in this game'; end if;

  select count(*) into v_count from public.{{slug}}_players where game_id = p_game_id;
...
$$;
grant execute on function public.{{slug}}_join_game(uuid) to authenticated;

-- Back-compat wrappers so the client can call either name.
create or replace function public.{{slug}}_join_open_game(p_game_id uuid)
returns void language plpgsql security definer as $$
begin perform public.{{slug}}_join_game(p_game_id); end; $$;
grant execute on function public.{{slug}}_join_open_game(uuid) to authenticated;

create or replace function public.{{slug}}_accept_invite(p_game_id uuid)
returns void language plpgsql security definer as $$
...
begin
  ...
  perform public.{{slug}}_join_game(p_game_id);
end; $$;
grant execute on function public.{{slug}}_accept_invite(uuid) to authenticated;

-- ── 10. Decline / cancel ──────────────────────────────────────
create or replace function public.{{slug}}_decline_invite(p_game_id uuid)
returns void language plpgsql security definer as $$
begin
...
$$;
grant execute on function public.{{slug}}_decline_invite(uuid) to authenticated;

-- Creator cancels their own waiting game before it starts.
create or replace function public.{{slug}}_cancel_invite(p_game_id uuid)
returns void language plpgsql security definer as $$
begin
  delete from public.{{slug}}_games
...
$$;
grant execute on function public.{{slug}}_cancel_invite(uuid) to authenticated;
...
create or replace function public.{{slug}}_advance_turn(p_game_id uuid)
returns void language plpgsql security definer as $$
declare
  v_game record; v_n int; v_total int := public.{{slug}}_total_turns();
...
begin
  ...
  select bool_and(p.turns_taken >= v_total)
    into v_all_done
    from public.{{slug}}_players p where p.game_id = p_game_id and not p.forfeited;
  if v_all_done is not false then
    perform public.{{slug}}_finalize_game(p_game_id);
    return;
  end if;
  ...
  for i in 1 .. v_n loop
    ...
    perform 1 from public.{{slug}}_players
...
create or replace function public.{{slug}}_finalize_game(
  p_game_id       uuid,
...
begin
  ...
  update public.{{slug}}_players
     set is_winner = (v_max is not null and total_score = v_max
                      and not forfeited and not (user_id = any(p_forced_losers)))
   where game_id = p_game_id;

  select count(*) into v_winners from public.{{slug}}_players where game_id = p_game_id and is_winner;
  select user_id into v_winner from public.{{slug}}_players where game_id = p_game_id and is_winner limit 1;
  ...
  for a in select user_id, is_winner from public.{{slug}}_players where game_id = p_game_id loop
    for b in select user_id from public.{{slug}}_players
              where game_id = p_game_id and user_id <> a.user_id loop
      insert into public.{{slug}}_matchups (player_id, opponent_id, wins, losses, ties)
      ...
      on conflict (player_id, opponent_id) do update set
        wins   = {{slug}}_matchups.wins   + excluded.wins,
        losses = {{slug}}_matchups.losses + excluded.losses,
        updated_at = now();
    end loop;
  end loop;
end;
...
-- everyone is done). With {{slug}}_total_turns() = 1, a 2-player game
...
--   4. bump turns_taken,
--   5. perform {{slug}}_advance_turn(p_game_id).
create or replace function public.{{slug}}_submit_turn(
  p_game_id uuid,
  p_score   int
) returns void language plpgsql security definer as $$
declare
...
begin
  ...
  select * into v_player from public.{{slug}}_players
  ...
  perform public.{{slug}}_advance_turn(p_game_id);
end;
$$;
grant execute on function public.{{slug}}_submit_turn(uuid, int) to authenticated;

-- ── 14. Forfeit (mark out; others continue, ≤1 left → finish) ─
create or replace function public.{{slug}}_forfeit_game(p_game_id uuid)
returns void language plpgsql security definer as $$
declare
...
begin
  ...
  if v_active <= 1 then
    perform public.{{slug}}_finalize_game(p_game_id);   -- last one standing wins
  elsif v_me.player_index = v_game.current_player_idx then
    perform public.{{slug}}_advance_turn(p_game_id);    -- it was my turn → hand off
  end if;
end;
$$;
grant execute on function public.{{slug}}_forfeit_game(uuid) to authenticated;

-- ── 15. Claim inactive win (boot idle current player, others continue) ─
create or replace function public.{{slug}}_claim_inactive_win(p_game_id uuid)
returns void language plpgsql security definer as $$
declare
...
$$;
grant execute on function public.{{slug}}_claim_inactive_win(uuid) to authenticated;

-- ── 16. Rematch ───────────────────────────────────────────────
-- Spawns a fresh game inviting all the same players (minus the caller).
create or replace function public.{{slug}}_rematch(p_game_id uuid)
returns uuid language plpgsql security definer as $$
declare
  v_uid     uuid := auth.uid();
  v_game    record;
...
begin
  ...
  if not public.{{slug}}_is_participant(p_game_id, v_uid) then raise exception 'Not a participant'; end if;

  select array_agg(user_id) into v_others
    from public.{{slug}}_players where game_id = p_game_id and user_id <> v_uid;

  return public.{{slug}}_create_game(v_others, v_game.max_players);
end;
$$;
grant execute on function public.{{slug}}_rematch(uuid) to authenticated;
...
drop function if exists public.{{slug}}_list_open_games();
create or replace function public.{{slug}}_list_open_games()
returns table(
  id                 uuid,
  created_by         uuid,
  ...
  select
    ...
    (select count(*)::int from public.{{slug}}_players pl where pl.game_id = g.id) as players_joined
  ...
  where g.status = 'waiting'
    ...
    and not exists (select 1 from public.{{slug}}_players pl where pl.game_id = g.id and pl.user_id = me.uid)
    and (
      (select count(*) from public.{{slug}}_players pl where pl.game_id = g.id)
      + (select count(*) from unnest(coalesce(g.invited_user_ids, '{}')) iu
          where not exists (select 1 from public.{{slug}}_players pl
                             where pl.game_id = g.id and pl.user_id = iu))
    ) < g.max_players
  order by g.created_at desc
  limit 50;
$$;
grant execute on function public.{{slug}}_list_open_games() to authenticated;
...
drop function if exists public.{{slug}}_pending_for(uuid);
create or replace function public.{{slug}}_pending_for(uid uuid)
returns table(count int, label text, url text)
language sql security definer stable as $$
  with invites as (
    select count(*)::int as n from public.{{slug}}_games
     where status = 'waiting'
       and (uid = any(coalesce(invited_user_ids, '{}')) or invited_user_id = uid)
       and not exists (select 1 from public.{{slug}}_players p
                        where p.game_id = {{slug}}_games.id and p.user_id = uid)
  ),
  turn as (
    select count(*)::int as n
      from public.{{slug}}_games g
      join public.{{slug}}_players p on p.game_id = g.id and p.user_id = uid
     where g.status = 'active' and p.player_index = g.current_player_idx and not p.forfeited
  )
  select n, 'Your turn'::text, '/{{slug}}/'::text from turn where n > 0
  union all
  select n, 'Invite'::text, '/{{slug}}/'::text from invites where n > 0
$$;
...
create or replace function public.{{slug}}_notify_event(p_type text, p_new jsonb, p_old jsonb default null)
returns void language plpgsql security definer as $$
begin
  begin
    perform net.http_post(
      -- REPLACE <PROJECT_REF>:
      url := 'https://<PROJECT_REF>.supabase.co/functions/v1/{{slug}}-push-notification',
  ...
  exception when others then
    raise warning '{{name}} % push trigger failed: %', p_type, SQLERRM;
  end;
end;
$$;

create or replace function public.{{slug}}_notify_game_invited()
returns trigger language plpgsql security definer as $$
begin
  if NEW.invited_user_id is null and coalesce(array_length(NEW.invited_user_ids, 1), 0) = 0 then
    return NEW;
  end if;
  perform public.{{slug}}_notify_event('game_invited', row_to_json(NEW)::jsonb);
  return NEW;
end;
$$;
drop trigger if exists on_{{slug}}_game_invited on public.{{slug}}_games;
create trigger on_{{slug}}_game_invited
  after insert on public.{{slug}}_games
  for each row
  when (NEW.invited_user_id is not null or NEW.invited_user_ids is not null)
  execute function public.{{slug}}_notify_game_invited();
...
$$;
drop trigger if exists on_{{slug}}_opponent_joined on public.{{slug}}_games;
create trigger on_{{slug}}_opponent_joined
  after update on public.{{slug}}_games
  for each row
  when (OLD.status = 'waiting' and NEW.status = 'active')
  execute function public.{{slug}}_notify_opponent_joined();

create or replace function public.{{slug}}_notify_turn_change()
returns trigger language plpgsql security definer as $$
begin
  perform public.{{slug}}_notify_event('turn_change', row_to_json(NEW)::jsonb, row_to_json(OLD)::jsonb);
  return NEW;
end;
$$;
drop trigger if exists on_{{slug}}_turn_change on public.{{slug}}_games;
create trigger on_{{slug}}_turn_change
  after update on public.{{slug}}_games
  for each row
  when (
  ...
  execute function public.{{slug}}_notify_turn_change();

create or replace function public.{{slug}}_notify_game_finished()
returns trigger language plpgsql security definer as $$
begin
  perform public.{{slug}}_notify_event('game_finished', row_to_json(NEW)::jsonb);
  return NEW;
end;
$$;
drop trigger if exists on_{{slug}}_game_finished on public.{{slug}}_games;
