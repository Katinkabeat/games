...
// {{name}} multiplayer game page — GENERIC N-player (2–4) shell ported
...
// {{slug}}_submit_turn RPC so the whole turn engine is exercisable
...
export default function MultiGamePage({ session, profile, isAdmin }) {
  ...
  // For waiting-state screens the other party may not be in {{slug}}_players
  ...
  // In waiting state the invitee isn't in {{slug}}_players yet, so resolve
  ...
  // {{slug}}_players yet). Fire at most once per gameId, only if I'm the
  ...
  // GAME-SPECIFIC: this calls the stub {{slug}}_submit_turn RPC with a
        ...
        {!isGameOver && iAmPlayer && !iForfeited && (game?.status === 'active' || isWaiting) && (
          <>
            {isWaiting ? (
              ...
              // stub {{slug}}_submit_turn RPC so you can watch the turn engine
              // work end-to-end (turns rotate between seats; the game finalizes
              // when everyone has taken {{slug}}_total_turns() turns and a
