...
// {{name}} MP lobby card. GENERIC N-player (2–4) lobby ported from
// Yahdle — works as-is for any SQ game using the {{slug}}_multiplayer.sql
// engine. Sections, in order:
//   • pending invites  (Accept / decline)
//   • sent invites     (waiting games I'm seated in; Cancel if mine)
//   • active games     (Wordy-style player chips with current-turn
//                       highlight, (n/n) count, "X ago", Resume, 🔔 nudge)
//   • open games       (joinable; Join)
//
// Lobby data is fetched once in LobbyPage via useMultiplayerLobby and
// passed in. Embedded player rows arrive on each game as the
// `{{slug}}_players` array (the supabase-js embed key).
//
// Chip colors reuse the shared sq-ui `wordy-*` palette on purpose — the
// `.dark .bg-wordy-*` overrides in sq-ui/globals.css give correct
// dark-mode chips for free. Do NOT swap in per-game palette classes.
export default function MultiplayerCard({
  ...
  // supabase-js embeds the players under the table name.
  const playersOf = (g) => g['{{slug}}_players'] ?? []
  ...
  function canNudge(g) {
    const me = playersOf(g).find(p => p.user_id === user?.id)
    ...
    <>
      <section className="card">
        ...
        <p className="text-sm opacity-80 mb-3">
          {/* GAME-SPECIFIC: write your multiplayer pitch. */}
          Invite up to 3 friends, or drop an open game for anyone to join.
        </p>
        <button
          type="button"
        ...
        <div className="space-y-2">
          ...
          {sentInvites.map(g => {
            ...
            const joined = playersOf(g).length
            const max = g.max_players ?? 2
          ...
          {activeGames.map(g => {
            const players = playersOf(g).slice().sort((a, b) => a.player_index - b.player_index)
