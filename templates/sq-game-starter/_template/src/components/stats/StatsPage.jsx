...
//  migrations:
//    supabase/migrations/{{slug}}_solo_results.sql
//    supabase/migrations/{{slug}}_solo_leaderboards.sql
//
//  Once those are applied AND your game writes a row into
//  {{slug}}_solo_results when a player finishes, this page will
...
function LeaderboardTab({ userId }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      supabase.rpc('{{slug}}_solo_leaderboard', { p_timeframe: timeframe, p_date: queryDate }),
      supabase.rpc('{{slug}}_solo_my_rank',     { p_timeframe: timeframe, p_date: queryDate }),
    ]).then(([lbRes, rankRes]) => {
      if (!active) return
...
// ─── My Stats tab — stub ─────────────────────────────────────
// TODO wire up {{name}}-specific stats (streak, win rate, lifetime
