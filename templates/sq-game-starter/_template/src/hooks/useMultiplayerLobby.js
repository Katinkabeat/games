...
// Loads the current user's {{name}} multiplayer lobby state:
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      if (now - lastExpireSweep.current > 5 * 60_000) {
        ...
        supabase.rpc('{{slug}}_expire_stale_invites').then(() => {}, () => {})
      }
      ...
      // already come back via {{slug}}_list_open_games, so we only need
