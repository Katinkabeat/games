...
- **Push notification on incoming requests:** (live as of 2026-04-25) edge function `sq-friend-request-notification` deployed via Supabase CLI 2.90.0; DB trigger `friendships_notify_on_insert` rewired. Each pending friendship insert fires fire-and-forget pg_net.http_post to the function, which sends a Web Push using the recipient's existing SideQuest/Wordy/Rungles subscription. Reuses the unified push-subscriptions architecture — no new permission prompts.
- **Note on tooling:** the Supabase Management API's `POST /v1/projects/{ref}/functions` endpoint does NOT actually bundle the function source — every deploy via that path returns BOOT_ERROR. Real deploys must go through `supabase functions deploy …`. The CLI is now installed via Scoop on Rae's machine.
