...
- **Push notification on incoming requests:** edge function source committed at `rae-side-quest/supabase/functions/sq-friend-request-notification/index.ts` and the trigger migration at `rae-side-quest/supabase/migrations/sq_friend_request_trigger.sql`. **Not active yet** — the function needs to be deployed via the Supabase CLI (the Management API's body-upload path fails with BOOT_ERROR even for hello-world, because it doesn't bundle for the Edge Runtime). Once Rae installs `supabase` CLI and runs `supabase functions deploy sq-friend-request-notification --no-verify-jwt`, the migration SQL re-creates the trigger and notifications start flowing. Friendship requests/accepts/removes work fine without it — notifications are an additive enhancement.
