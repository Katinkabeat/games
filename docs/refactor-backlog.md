...
## Candidates surfaced 2026-07-12 (notification deep-link routing)

- [ ] **Migrate Rungles + Snibble to react-router (they're the last two off-pattern games).** Wordy, Yahdle, Oublex — and the `sq-game-starter` scaffold — all route with react-router + path params (`/game/:id`, `/multi/:gameId`), which is why their notification deep-links land on the right board reliably. Rungles and Snibble never adopted it: Rungles uses hand-rolled view-state (`setView('multi')` + a read-once `?game=` effect that then strips the query), and Snibble routes its **entire** app through query strings (`?play=daily`, `?view=sanctuary`, `?match=id`) via a single `goTo()` pushState helper. That divergence is what caused the 2026-07-12 "notification opens the same board over and over" bug. **The bug itself is already fixed** (that session made both games' deep-links path-based + re-read on focus/visibility — targeted, low-risk, left the custom routing in place). This item is the *proper* follow-up: adopt react-router in both so all five games are architecturally identical. **Risk:** Rungles is medium (contained view set; the awkward bit is `statsReturnView` "go back to where I came from" logic). **Snibble is high** — it's a full navigation rewrite touching every view + every back button (`LobbyView`/`GameView`/`SanctuaryView`/`MatchView`/`StatsPage`/`AdminPanel`), against a game that otherwise works fine. Do these as deliberate, separately-verified cards during a big refactor — not bolted onto a bugfix. Scaffold is already correct, so no future game inherits this.
